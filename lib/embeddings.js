// Nel tuo file vector-store.js

export async function searchDocuments(queryText, topK = 3) {
    try {
        console.log('--- 1. TENTATIVO DI EMBEDDING ---');
        
        const queryVector = await createEmbedding(queryText); // Chiamata a HF

        console.log('--- 2. EMBEDDING RIUSCITO. Dimensione:', queryVector.length, '---');
        
        // 🚨 NON CHIAMARE UPSTASH
        return [
            { text: "Test OK", score: 1.0, source: `Dimensione: ${queryVector.length}` }
        ]; 
        
    } catch (error) {
        console.error('❌ ERRORE NEL TEST HF:', error);
        throw error; 
    }
}
